...
/**
 * A game line starts with "Game $ID", so the id begins at index 5
 */
static final int GAME_ID_INDEX = 5;

void main() {
	long result = sumSmallestBagPowers(input);
	assert result == 2286;
	System.out.println(result);
}

long sumSmallestBagPowers(String input) {
	return input
			.lines()
			.map(this::parseGame)
			.map(Game::smallestBag)
...
private Game parseGame(String line) {
	...
	var idString = gameLine[0].substring(GAME_ID_INDEX);
...
record Game(int id, List<CubeSet> iterations) {
	...
	CubeSet smallestBag() {
		...
		int green = 0;
		int blue = 0;
		for (CubeSet iteration : iterations) {
			red = Math.max(red, iteration.red());
			green = Math.max(green, iteration.green());
			blue = Math.max(blue, iteration.blue());
		}
		return new CubeSet(red, green, blue);
	}
